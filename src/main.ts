...
app.use((req: MyUserRequest, res: express.Response, next: express.NextFunction) => {
...
app.get('/mainpage', async (req: MyUserRequest, res) => {
  ...
  const schedule = (await pool.query(`SELECT users.id as user_id, firstname, day, start_at, end_at FROM users JOIN schedules ON (users.id=user_id);`)).rows;
...
app.get('/userPage/:id', async (req: MyUserRequest, res) => {
  ...
  const scheduleUser = (await pool.query(`SELECT * FROM schedules WHERE user_id=${id};`)).rows;
